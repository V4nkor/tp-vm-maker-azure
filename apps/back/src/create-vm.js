...
const sku = "18.04-LTS";
const vmImageVersionNumber = "latest";
...
async function findVMImage() {
  ...
  /* for await (const item of computeClient.virtualMachineImages.list(
    ...
    listResult.push(
        await computeClient.virtualMachineImages.list(location, publisher, offer, sku)
    );
...
async function getPublicIP(vmInfo) {
    const nicId = vmInfo.networkProfile.networkInterfaces[0].id;
    const nicInfo = await networkClient.networkInterfaces.get(
        resourceGroupName,
        nicId
    );
    const ipId = nicInfo.ipConfigurations[0].publicIPAddress.id;
